...
小米云端服务 Node.js 客户端，使用原生 HTTP 请求解决跳转Cookies问题。
...
```bash
# 列出所有设备
node bin/micli.js list

# 按名称过滤设备
node bin/micli.js list Light

# 列出完整设备信息（含虚拟模型和华米设备）
node bin/micli.js list full true 1

# 查看 MiNA 设备列表（小爱音箱）
node bin/micli.js mina
```
...
node bin/micli.js <did> 1,1-2,1-3,1-4,2-1,2-2,3

# 设置属性（值前加 # 表示数字）
node bin/micli.js <did> 2=#60,2-2=#false,3=test

# 执行动作
node bin/micli.js <did> 5 Hello
node bin/micli.js <did> 5-4 Hello #1

# 使用 #NA 表示无参数
node bin/micli.js <did> 2 #NA
```
...
```bash
# 查看所有 spec
node bin/micli.js spec

# 按型号搜索
node bin/micli.js spec speaker

# 指定设备型号，x08cRedmi小爱触屏音箱 8
node bin/micli.js spec xiaomi.wifispeaker.x08c

# 指定 URN
node bin/micli.js spec urn:miot-spec-v2:device:speaker:0000A015:xiaomi-lx04:1

# 输出格式（text/python/json）
node bin/micli.js spec xiaomi.wifispeaker.lx04 json
```
...
```bash
# 调用 MiIO API
node bin/micli.js <did> /home/device_list '{"getVirtualModel":false,"getHuamiDevices":1}'

# 调用 MIoT 接口
node bin/micli.js <did> action '{"did":"267090026","siid":5,"aiid":1,"in":["Hello"]}'
```
...
```bash
# TTS 播报
node bin/micli.js message "你好，世界"

# 播放音频 URL
node bin/micli.js play https://example.com/music.mp3

# 单曲循环
node bin/micli.js loop https://example.com/music.mp3

# 暂停/停止
node bin/micli.js pause
node bin/micli.js stop

# 播放列表（文件每行一个 URL）
node bin/micli.js play_list urls.txt

# 播放 Suno 热门歌曲
node bin/micli.js suno

# 随机播放 Suno 热门歌曲
node bin/micli.js suno_random
```
...
```bash
# 解密 RC4 加密数据
node bin/micli.js decode <ssecurity> <nonce> <data> [gzip]
```
...
const { MiIOService, MiNAService, createAccount } = require('./src');
...
require('dotenv').config();
const path = require('path');
const { MiNAService, MiNAListener, createAccount } = require('./src');
