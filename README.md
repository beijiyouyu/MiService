...
MUSIC_API=https://your-music-api.com/search（可选，音乐搜索接口，接收歌名返回音频URL列表）
```
...
async function searchMusic(apiUrl, musicName) {
  ...
  if (Array.isArray(json.data)) {
    return json.data
      ...
      .map(item => ({ url: item.url, duration: (item.duration || 180000) / 1000 }));
  }
  // 接口返回单条
  const audioUrl = json.url || (json.data && json.data.url);
  const duration = (json.data && json.data.duration) || 180000;
  if (!audioUrl) throw new Error('接口未返回音频地址: ' + JSON.stringify(json));
...
async function playList(mina, deviceId, tracks) {
  ...
  for (let i = 0; i < tracks.length; i++) {
    ...
    console.log(`[${i + 1}/${tracks.length}] 🎶 播放: ${url} (${Math.round(duration)}s)`);
...
配置 `MUSIC_API` 后，说"我想听隐形的翅膀"会自动调用接口搜索并播放。接口需接收 `musicName` 参数，返回单个音频或播放列表：

```bash
# 接口调用示例
curl "https://your-music-api.com/search?musicName=隐形的翅膀"

# 返回单条音频：
# { "url": "https://xxx.mp3", "duration": 240000 }

# 返回播放列表（自动按顺序播放）：
# { "data": [
#   { "url": "https://xxx.mp3", "duration": 240000 },
#   { "url": "https://yyy.mp3", "duration": 180000 }
# ] }
```

> `duration` 单位为毫秒，缺省时默认 180000（3 分钟）。播放列表按顺序逐首播放，每首播放完毕后自动切下一首。
...
listener.on('music', async (msg) => {
  ...
  if (msg.command === 'play' && msg.songName) {
    await mina.stop_tts(deviceId);
    const tracks = await searchMusic(msg.songName); // 返回 [{ url, duration }]
    // 逐首播放
    for (const { url, duration } of tracks) {
      await mina.play_by_url(deviceId, url);
